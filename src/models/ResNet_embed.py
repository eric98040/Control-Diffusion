...
class ResNet_embed(nn.Module):
    def __init__(self, block, num_blocks, nc=NC, dim_embed=DIM_EMBED, ngpu=1):
        ...
        self.main = nn.Sequential(
            ...
            self._make_layer(block, 64, num_blocks[0], stride=2),  # h=h/2 16
            self._make_layer(block, 128, num_blocks[1], stride=2),  # h=h/2 8
            self._make_layer(block, 256, num_blocks[2], stride=2),  # h=h/2 4
            # self._make_layer(block, 512, num_blocks[3], stride=),  # h=h/2 4
        ...
        self.h2y = nn.Sequential(nn.Linear(dim_embed, 2), nn.ReLU())
