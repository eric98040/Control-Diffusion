...
class ResidualBlock(TimestepBlock):
    def __init__(
        ...
        self.tc_mlp = nn.Sequential(
            nn.SiLU(),
            nn.Linear(
                int(time_channels) + int(cond_channels),
...
class Unet(nn.Module):
    def __init__(
        ...
        attention_resolutions=(8,),  # 8, 16
        dropout=0,
        channel_mult=(1, 2, 4, 8),  # 수정: 4개의 채널 배수
        conv_resample=True,
        num_heads=4,
        ...
        self.null_classes_emb = nn.Parameter(
            -1 * torch.abs(torch.randn(cond_embed_dim)), requires_grad=False
        )
        ...
        for level, mult in enumerate(channel_mult):
            ...
            ):  # 다운샘플링을 3번 수행 (channel_mult 길이-1)
        ...
        self.out = nn.Sequential(
            ...
            nn.SiLU(),
            nn.Conv2d(ch, out_channels, kernel_size=3, padding=1),  # 수정: ch 사용
            # torch.clamp(min=0, max=1),
        )

    def forward(
        self, x, timesteps, classes, cond_drop_prob=None, return_null_indx=False
    ):
