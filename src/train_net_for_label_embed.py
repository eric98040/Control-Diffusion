...
def train_net_embed(
    ...
    if path_to_ckpt is not None and resume_epoch > 0:
        save_file = (
            path_to_ckpt
            + "/embed_x2y_ckpt_in_train/embed_x2y_checkpoint_epoch_{}.pth".format(
                resume_epoch
            )
        )
    ...
    for epoch in range(resume_epoch, epochs):
        ...
        for _, batch in enumerate(trainloader):
            ...
            batch_train_images = batch["design"].type(torch.float).cuda()
            batch_train_labels = batch["labels"].type(torch.float).cuda()
            # batch_train_labels = batch["labels"].type(torch.float).view(-1, 1).cuda()
        ...
        if testloader is None:
            ...
            with torch.no_grad():
                ...
                for batch_test_images, batch_test_labels in testloader:
                    ...
                    batch_test_labels = (
                        batch_test_labels.type(torch.float).view(-1, 1).cuda()
                    )
...
def train_net_y2h(
    ...
    for epoch in range(epochs):
        ...
        for _, batch_labels in enumerate(trainloader):
            ...
            batch_gamma = np.random.normal(0, 0.2, (batch_size_curr, 2))
            batch_gamma = torch.from_numpy(batch_gamma).type(torch.float).cuda()
